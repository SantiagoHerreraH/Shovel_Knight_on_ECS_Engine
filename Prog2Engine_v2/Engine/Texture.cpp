...
void Texture::Draw( const Point2f& locationBottomLeftInScreenPosition, const Rectf& rectangleViewForSpriteSheet ) const
{
	const float epsilon{ 0.001f };
	if ( !m_CreationOk )
	{
		if (!(rectangleViewForSpriteSheet.width > epsilon && rectangleViewForSpriteSheet.height > epsilon)) // No srcRect specified
		{
			DrawFilledRect(Rectf{ locationBottomLeftInScreenPosition.x, locationBottomLeftInScreenPosition.y, m_Width, m_Height });
		}
		else
		{
			DrawFilledRect(Rectf{ locationBottomLeftInScreenPosition.x, locationBottomLeftInScreenPosition.y, rectangleViewForSpriteSheet.width, rectangleViewForSpriteSheet.height });
		}
	}
	else
	{
		Rectf dstRect{ locationBottomLeftInScreenPosition.x, locationBottomLeftInScreenPosition.y, rectangleViewForSpriteSheet.width, rectangleViewForSpriteSheet.height };
		if (!(rectangleViewForSpriteSheet.width > epsilon && rectangleViewForSpriteSheet.height > epsilon)) // No srcRect specified
		{
			dstRect.width = m_Width;
			dstRect.height = m_Height;
		}
		Draw( dstRect, rectangleViewForSpriteSheet );
	}
}

void Texture::Draw( const Rectf& locationAndScaleOfTextureInScreen, const Rectf& rectangleViewForSpriteSheet ) const
{
	const float epsilon{ 0.001f };
	if ( !m_CreationOk )
	{
		DrawFilledRect( locationAndScaleOfTextureInScreen );
		return;
	}
	...
	else // srcRect specified
	{
		...
		textLeft = rectangleViewForSpriteSheet.left / m_Width;
		textRight = ( rectangleViewForSpriteSheet.left + rectangleViewForSpriteSheet.width ) / m_Width;
		textTop = rectangleViewForSpriteSheet.bottom / m_Height;
		textBottom = (rectangleViewForSpriteSheet.bottom + rectangleViewForSpriteSheet.height) / m_Height;

		defaultDestHeight = rectangleViewForSpriteSheet.height;
		defaultDestWidth = rectangleViewForSpriteSheet.width;
	}

	// Determine vertex coordinates
	float vertexLeft{ locationAndScaleOfTextureInScreen.left };
	float vertexBottom{ locationAndScaleOfTextureInScreen.bottom };
	float vertexRight{};
	float vertexTop{};
	if ( !( locationAndScaleOfTextureInScreen.width > 0.001f && locationAndScaleOfTextureInScreen.height > 0.001f ) ) // If no size specified use default size
	{
	...
	{
		vertexRight = vertexLeft + locationAndScaleOfTextureInScreen.width;
		vertexTop = vertexBottom + locationAndScaleOfTextureInScreen.height;
